...
import React from 'react'
  ...
  const Orders = ({ cartItems, onBack }: OrdersProps) => {
    ...
    const totalAmount = cartItems.reduce((total, item) => total + calculateTotalAmount(item.dish.price, item.count), 0);
    return (
<Grid item xs={6} sm={4}>
      <Box
        sx={{
      ...
      >
        ...
        {cartItems.length > 0 ? (
          cartItems.map((item) => (
            <Box key={item.dish.id} sx={{ display: 'flex', justifyContent: 'space-between', width: '100%', paddingTop: '20px' }}>
              ...
              <Typography fontSize="14px" color="black" align="right" padding="10px 40px">
                x{item.count} &nbsp; &nbsp;Aed {calculateTotalAmount(item.dish.price, item.count).toFixed(2)}
              </Typography>
            </Box>
          ))
        ) : (
